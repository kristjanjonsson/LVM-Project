...
def main():
    ...
    N = 10
    diff = True
    (trainLabeled, trainBinned) = toBins(trainData, minT, maxT, N, nUsers, nItems, diff)
    (testLabeled, testBinned) = toBins(testData, minT, maxT, N, nUsers, nItems, diff)
...
def toBins(xs, minT, maxT, n, nUsers, nItems, diff):
    edges = np.linspace(minT-1, maxT, num=n+1)
    binned = np.zeros((nUsers, nItems, n), dtype=np.int32)
    labeled = np.copy(xs)
    for i in range(1, len(edges)):
        if diff:
            inds = np.logical_and(xs[:,3] <= edges[i], xs[:,3] > edges[i-1])
        else:
            inds = xs[:,3] <= edges[i]
        # cvt to matrix
        users = xs[inds, 0] - 1
